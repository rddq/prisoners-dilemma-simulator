...
class AnalysisHelper:
    ...
    @staticmethod
    def run_analysis_all_matchups(results):
        i = 1
        for result in results:
            print("You Against Player "+str(i))
            AnalysisHelper.run_analysis_all_games(result)
            i += 1

    @staticmethod
    def run_analysis_all_games(results):
        for result in results:           
            AnalysisHelper.run_analysis_on_game(result)

    @staticmethod
    def run_analysis_on_game(results):
        score_totals = AnalysisHelper.total_score(results)
    ...
    @staticmethod
    def add_to_csv(data_for_csv,results_game,game,player_1_type,player_2_type,game_name):
        data_to_add = []
        data_to_add.append(player_1_type)
        data_to_add.append(player_2_type)
        data_to_add.append(game.rounds)
        data_to_add.append(game_name)
